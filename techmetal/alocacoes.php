...
<div class="container">
  ...
  <path d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z"/>
                ...
                <div class="card-body">
                    ...
                    $sql = "SELECT * FROM alocacoes ORDER BY data_movimento";
                    $consulta = $pdo->query($sql);
                    $listaalocacoes = $consulta->fetchAll(PDO::FETCH_ASSOC);
                    ...
                    if (count($listaalocacoes) > 0) {
                        ...
                        foreach ($listaalocacoes as $item) {
                            $id = $item['id'];
                            echo "<tr>";
                            echo "<td>" . htmlspecialchars($item['id']) . "</td>";
                            echo "<td>" . htmlspecialchars($item['maquina']) . "</td>";
                            echo "<td>" . htmlspecialchars($item['data_movimento']) . "</td>";
                            echo "<td>" . htmlspecialchars($item['setor_atual']) . "</td>";
                            echo "<td>" . htmlspecialchars($item['setor_anterior']) . "</td>";
                          ...
                          </svg></a> | <a href ='excluir_alocacoes.php?id=$id'><svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-trash-fill text-danger' viewBox='0 0 16 16'>
                        ...
                        echo "<p class='text-danger'><b>NÃO MAQUINAS REALOCADAS NO MOMENTO!</b></p>";
                    }
                    ?>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-hidden="true">
  ...
  <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7m.5-5v1h1a.5.5 0 0 1 0 1h-1v1a.5.5 0 0 1-1 0v-1h-1a.5.5 0 0 1 0-1h1v-1a.5.5 0 0 1 1 0m-2-6a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4"/>
  <path d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z"/>
</svg>&nbsp; &nbsp; <h5 class="modal-title">MAQUINAS REALOCADAS</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>

            <div class="modal-body">
                <form action="cadastro_alocacoes.php" method="POST">
                ...
                <input type="date" name="data_movimento" class="form-control" required/>
                <br/> 
                <label class="form-label">SETOR ATUAL</label>
                <input type="text" name="setor_atual" class="form-control" required/>
                <br/> 
                <label class="form-label">SETOR ANTERIOR</label>
                <input type="text" name="setor_anterior" class="form-control" required/>
                <br/>
                <label class="form-label">RESPONSAVEL</label>
                <input type="text" name="responsavel" class="form-control" required/>
                <br/>
                
                <button type="submit" class="btn btn-outline-success">INFORMAR</button>    
                </form>
            </div>
...
<div class="modal fade" id="modalEditar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7m.5-5v1h1a.5.5 0 0 1 0 1h-1v1a.5.5 0 0 1-1 0v-1h-1a.5.5 0 0 1 0-1h1v-1a.5.5 0 0 1 1 0m-2-6a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4"/>
  <path d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z"/>
</svg>&nbsp; &nbsp; <h5 class="modal-title" id="modalEditar">EDIÇÂO DE ALOCAÇOES</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
           <div class="modal-body">
           <form action="editar_alocacoes.php" method="POST">
            <input type="hidden" id="edit_id" name="id"/>
                <label class="form-label">MAQUINA</label>
                <input type="text" name="maquina" class="form-control" id="edit_maquina" required/>
                <br/> 
                <label class="form-label">DATA DE MOVIMENTAÇÃO</label>
                <input type="date" name="data_movimento" class="form-control" id="edit_data_movimento" required/>
                <br/> 
                <label class="form-label">SETOR ATUAL</label>
                <input type="text" name="setor_atual" class="form-control" id="edit_setor_atual " required/>
                <br/> 
                <label class="form-label">SETOR ANTERIOR</label>
                <input type="text" name="setor_anterior" class="form-control" id="edit_setor_anterior " required/>
                <br/> 
                <label class="form-label">RESPONSAVEL</label>
                <input type="text" name="reponsavel" class="form-control" id="edit_responsavel " required/>
                <br/> 
...
<script>
    document.getElementById('modalEditar').addEventListener('show.bs.modal', function(event){
        ...
        fetch('buscar_alocacoes.php?id=' +id)
           ...
           .then(data=>{
               ...
               document.getElementById('edit_setor_atual').value = data.setor_atual;
               document.getElementById('edit_setor_anterior').value = data.setor_anterior;
